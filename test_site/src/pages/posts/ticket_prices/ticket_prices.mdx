...
That means this pattern could shift if we just look at the most recent prices for each game a visiting team plays. You can toggle that with the buttons below. You will notice
that not much changes, other than the overall z-scores decreasing, which makes sense as tickets closer to gametime should be cheaper than the average price of the same tickets.
...
These dates don't really have a particular importance, except that the June 11 game occured before COVID capacity restrictions were lifed (June 15). Because of the lower 
...
expensive than Satuday games, likely due to the fact that many Sunday games are day games. And not many people would willingly pay to sit in the blistering sun for a couple
...
The largest limiation is the data set itself. It only just about one summer of baseball -- albeit an unusual summer with COVID still around. As such, it is difficult to see
...
One potential area for growth could be a machine learning algorthim to predict ticket prices with several of the predictors we have discussed. Again, the only issue is the size
of the data set. 
